...
async def restart(message: Message):
    ...
    logger.info(lang.log_command_executed.format(command='restart', user_id=message.from_user.id))

    try:
        # Notify user that restart is starting
        restart_msg = await message.answer("🔄 <b>Restarting Bot Application...</b>\n\n⏳ Initializing restart process...", parse_mode="HTML")
        ...
        try:
            ...
            await restart_msg.edit_text(f"❌ <b>Failed to start new process!</b>\n\nError: {e}", parse_mode="HTML")
            return
        ...
        if new_process.poll() is None:
            ...
            await restart_msg.edit_text("✅ <b>Bot Restarted Successfully!</b>\n\n🔄 New instance is now running.\n⏹️ Shutting down old instance...", parse_mode="HTML")
            ...
            await restart_msg.edit_text(f"❌ <b>Restart Failed!</b>\n\nNew process exited with code: {exit_code}", parse_mode="HTML")
            logger.error(f"Restart failed: New process exited with code {exit_code}")

    except Exception as e:
        logger.error(f"Restart error: {e}")
        await message.answer("❌ Failed to restart the application. Check logs for details.")