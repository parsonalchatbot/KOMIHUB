...
async def add_admin(message: Message):
    ...
    logger.info(lang.log_command_executed.format(command='add_admin', user_id=message.from_user.id))
    ...
    if message.reply_to_message:
        ...
        if len(args) < 2:
            await message.answer("Usage: /add_admin [user_id or reply] [admin_type]\nAdmin types: admins, elders, gc_admins, ch_admins")
            return
        try:
...
async def remove_admin(message: Message):
    ...
    logger.info(lang.log_command_executed.format(command='remove_admin', user_id=message.from_user.id))
    ...
    if message.reply_to_message:
        ...
        if len(args) < 2:
            await message.answer("Usage: /remove_admin [user_id or reply] [admin_type]\nAdmin types: admins, elders, gc_admins, ch_admins")
            return
        try:
            target_user_id = int(args[1])
            target_user = await message.bot.get_chat_member(message.chat.id, target_user_id)
...
async def list_admins(message: Message):
    ...
    logger.info(lang.log_command_executed.format(command='list_admins', user_id=message.from_user.id))
