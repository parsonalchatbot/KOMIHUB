    ...
    try:
        ...
        if db.is_command_disabled(command_text):
            await message.answer(
                "❌ This command is currently disabled by administrators."
            )
            return
        ...
        if command_text in ["start", "help", "ping", "info", "stats"]:
            ...
            for keyword in ["dl", "download", "music", "video", "audio"]
        ):
            ...
            # QR code related
            response = (
                f"❓ <b>QR command not found:</b> <code>/{command_text}</code>\n\n"
            )
            response += "🔳 <b>Available:</b> /qrcode <text> - Generate QR codes\n"

        elif any(
            keyword in command_text for keyword in ["admin", "manage", "ban", "kick"]
        ):
            # Admin commands
            response = (
                f"❓ <b>Admin command not found:</b> <code>/{command_text}</code>\n\n"
            )
